...
data_path = os.path.join(pathlib.Path(file_dir).parent, "database/druglikeness")
_mcf = pd.read_csv(os.path.join(data_path, "mcf.csv"))
_pains = pd.read_csv(
    os.path.join(data_path, "wehi_pains.csv"), names=["smarts", "names"]
)
_filters = [
    Chem.MolFromSmarts(x) for x in pd.concat([_mcf, _pains], sort=True)["smarts"].values
]

inf = open(os.path.join(data_path, "pains.txt"), "r")
