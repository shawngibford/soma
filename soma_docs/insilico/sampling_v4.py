...
def generate_to_csv(model, prior, n_samples):
    g_samples = generate_bulk_samples(
        model, n_samples, 1000, 2, prior=prior, verbose=True, unique=True,
    )
    ...
    file_name = (
        f"{saved_compounds_dir}/model_type_valid_unique_samples{run_date_time}.csv"
    )
    compund_to_csv(
        results_analysis, file_path=file_name,
    )
    print(
        f"diversity_fraction={results_analysis.diversity_fraction}, filter_fraction={results_analysis.filter_fraction}, unique_fraction={results_analysis.unique_fraction}"
    )
    return (results_analysis, file_name)
...
if __name__ == "__main__":
    print(model_type, n_samples)
    submited_workflows, not_submitted = send_to_chemistry42(
        model_type, model, prior, n_samples
    )
    total = len(not_submitted)
    print(f"{model_type} not submitted {not_submitted}")
    n = 0
    while total > 0:
        submited_workflows_loop, not_submitted_loop = submit_smiles_to_chemistry42(
            model_type, not_submitted
        )
    ...
    save_obj(
        submited_workflows,
        f"{saved_compounds_dir}/{model_type}_{n_samples}_{run_date_time}.pkl",
    )
