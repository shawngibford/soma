...
class RewardAPI:
    def __init__(self,username,password, base_url="https://rip.chemistry42.com", db_path="workflows.db"):
    ...
    def save_workflow_id(self, name, workflow_uuid):
        ...
        conn = sqlite3.connect(self.db_path)
        c = conn.cursor()
        c.execute("INSERT OR IGNORE INTO workflows (name, workflow_uuid, timestamp) VALUES (?, ?, ?)",
    ...
    def get_all_workflows(self):
        ...
        result = c.fetchall()
        conn.close()
        return [{"id": row[0], "name": row[1], "workflow_uuid": row[2], "timestamp": row[3]} for row in result]
