...
from orquestra.qml.optimizers.th import TorchOptimizerConfig, AdamConfig
...
class MolPAT(TorchGenerativeModel):
    ...
    def _raise_error_if_invalid_train_batch(self, batch: Batch[torch.Tensor]) -> None:
        ...
        if batch.targets is None:
            raise ValueError("Expected targets to be provided for training. Please set samples from prior distribution as targets.")
    ...
    def _make_x0(self, n_samples: int) -> torch.LongTensor:
        ...
        return torch.full((n_samples, 1), self.start_token_index, dtype=torch.long, device=self._device)
    ...
    def _generate_w_probs(
        ...
        outputs = torch.zeros((n_samples, self.seq_len), device=self._device, dtype=torch.long)
    ...
    def make_lookahead_mask(self, x: torch.Tensor) -> torch.Tensor:
        ...
        return torch.triu(torch.ones(x.size(1), x.size(1)), diagonal=1).bool().to(device=self._device)
